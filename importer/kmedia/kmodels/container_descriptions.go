...
func (containerDescriptionL) LoadContainer(e boil.Executor, singular bool, maybeContainerDescription interface{}) error {
	...
	if singular {
		...
		slice = *maybeContainerDescription.(*[]*ContainerDescription)
		count = len(slice)
	}
...
func (o *ContainerDescription) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"container_descriptions\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"container_descriptions\" DEFAULT VALUES"
		}
...
func (o ContainerDescriptionSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"container_descriptions\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, containerDescriptionPrimaryKeyColumns, len(o)))
...
func (o *ContainerDescription) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"container_descriptions\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(containerDescriptionType, containerDescriptionMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o ContainerDescriptionSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"container_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, containerDescriptionPrimaryKeyColumns, len(o))
...
func (o *ContainerDescriptionSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"container_descriptions\".* FROM \"container_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, containerDescriptionPrimaryKeyColumns, len(*o))
