...
func (o *CkeditorAsset) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ckeditor_assets\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ckeditor_assets\" DEFAULT VALUES"
		}
...
func (o CkeditorAssetSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"ckeditor_assets\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, ckeditorAssetPrimaryKeyColumns, len(o)))
...
func (o *CkeditorAsset) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"ckeditor_assets\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(ckeditorAssetType, ckeditorAssetMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o CkeditorAssetSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"ckeditor_assets\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ckeditorAssetPrimaryKeyColumns, len(o))
...
func (o *CkeditorAssetSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"ckeditor_assets\".* FROM \"ckeditor_assets\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ckeditorAssetPrimaryKeyColumns, len(*o))
