...
func (fileAssetDescriptionL) LoadFile(e boil.Executor, singular bool, maybeFileAssetDescription interface{}) error {
	...
	if singular {
		...
		slice = *maybeFileAssetDescription.(*[]*FileAssetDescription)
		count = len(slice)
	}
...
func (o *FileAssetDescription) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"file_asset_descriptions\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"file_asset_descriptions\" DEFAULT VALUES"
		}
...
func (o FileAssetDescriptionSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"file_asset_descriptions\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, fileAssetDescriptionPrimaryKeyColumns, len(o)))
...
func (o *FileAssetDescription) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"file_asset_descriptions\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(fileAssetDescriptionType, fileAssetDescriptionMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o FileAssetDescriptionSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"file_asset_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, fileAssetDescriptionPrimaryKeyColumns, len(o))
...
func (o *FileAssetDescriptionSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"file_asset_descriptions\".* FROM \"file_asset_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, fileAssetDescriptionPrimaryKeyColumns, len(*o))
