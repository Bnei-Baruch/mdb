...
func (rolesUserL) LoadUser(e boil.Executor, singular bool, maybeRolesUser interface{}) error {
	...
	if singular {
		...
		slice = *maybeRolesUser.(*[]*RolesUser)
		count = len(slice)
	}
...
func (o *RolesUser) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"roles_users\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"roles_users\" DEFAULT VALUES"
		}
...
func (o *RolesUser) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"roles_users\"", updateOnConflict, ret, update, conflict, insert)
