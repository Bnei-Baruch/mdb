...
func (o *LabelDescription) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"label_descriptions\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"label_descriptions\" DEFAULT VALUES"
		}
...
func (o LabelDescriptionSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"label_descriptions\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, labelDescriptionPrimaryKeyColumns, len(o)))
...
func (o *LabelDescription) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"label_descriptions\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(labelDescriptionType, labelDescriptionMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o LabelDescriptionSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"label_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, labelDescriptionPrimaryKeyColumns, len(o))
...
func (o *LabelDescriptionSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"label_descriptions\".* FROM \"label_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, labelDescriptionPrimaryKeyColumns, len(*o))
