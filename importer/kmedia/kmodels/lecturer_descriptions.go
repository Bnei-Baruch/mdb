...
func (o *LecturerDescription) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"lecturer_descriptions\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"lecturer_descriptions\" DEFAULT VALUES"
		}
...
func (o LecturerDescriptionSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"lecturer_descriptions\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, lecturerDescriptionPrimaryKeyColumns, len(o)))
...
func (o *LecturerDescription) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"lecturer_descriptions\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(lecturerDescriptionType, lecturerDescriptionMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o LecturerDescriptionSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"lecturer_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, lecturerDescriptionPrimaryKeyColumns, len(o))
...
func (o *LecturerDescriptionSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"lecturer_descriptions\".* FROM \"lecturer_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, lecturerDescriptionPrimaryKeyColumns, len(*o))
