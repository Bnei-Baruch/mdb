...
func (o *Catalog) CatalogDescriptions(exec boil.Executor, mods ...qm.QueryMod) catalogDescriptionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"catalog_descriptions\".\"catalog_id\"=?", o.ID),
	)
...
func (o *Catalog) ParentCatalogs(exec boil.Executor, mods ...qm.QueryMod) catalogQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"catalogs\".\"parent_id\"=?", o.ID),
	)
...
func (o *Catalog) ContainerDescriptionPatterns(exec boil.Executor, mods ...qm.QueryMod) containerDescriptionPatternQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"catalogs_container_description_patterns\" on \"container_description_patterns\".\"id\" = \"catalogs_container_description_patterns\".\"container_description_pattern_id\""),
		qm.Where("\"catalogs_container_description_patterns\".\"catalog_id\"=?", o.ID),
	)

	query := ContainerDescriptionPatterns(exec, queryMods...)
	queries.SetFrom(query.Query, "\"container_description_patterns\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"container_description_patterns\".*"})
	}
...
func (o *Catalog) Containers(exec boil.Executor, mods ...qm.QueryMod) containerQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"catalogs_containers\" on \"containers\".\"id\" = \"catalogs_containers\".\"container_id\""),
		qm.Where("\"catalogs_containers\".\"catalog_id\"=?", o.ID),
	)
...
func (o *Catalog) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"catalogs\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"catalogs\" DEFAULT VALUES"
		}
...
func (o CatalogSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"catalogs\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, catalogPrimaryKeyColumns, len(o)))
...
func (o *Catalog) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"catalogs\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(catalogType, catalogMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o CatalogSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"catalogs\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, catalogPrimaryKeyColumns, len(o))
...
func (o *CatalogSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"catalogs\".* FROM \"catalogs\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, catalogPrimaryKeyColumns, len(*o))
