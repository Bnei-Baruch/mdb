...
func (catalogDescriptionL) LoadCatalog(e boil.Executor, singular bool, maybeCatalogDescription interface{}) error {
	...
	if singular {
		...
		slice = *maybeCatalogDescription.(*[]*CatalogDescription)
		count = len(slice)
	}
...
func (o *CatalogDescription) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"catalog_descriptions\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"catalog_descriptions\" DEFAULT VALUES"
		}
...
func (o CatalogDescriptionSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"catalog_descriptions\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, catalogDescriptionPrimaryKeyColumns, len(o)))
...
func (o *CatalogDescription) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"catalog_descriptions\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(catalogDescriptionType, catalogDescriptionMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o CatalogDescriptionSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"catalog_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, catalogDescriptionPrimaryKeyColumns, len(o))
...
func (o *CatalogDescriptionSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"catalog_descriptions\".* FROM \"catalog_descriptions\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, catalogDescriptionPrimaryKeyColumns, len(*o))
