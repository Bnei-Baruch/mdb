...
func (o *VirtualLesson) Containers(exec boil.Executor, mods ...qm.QueryMod) containerQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"containers\".\"virtual_lesson_id\"=?", o.ID),
	)
...
func (virtualLessonL) LoadContainers(e boil.Executor, singular bool, maybeVirtualLesson interface{}) error {
	...
	if singular {
		...
		slice = *maybeVirtualLesson.(*[]*VirtualLesson)
		count = len(slice)
	}
...
func (o *VirtualLesson) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"virtual_lessons\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"virtual_lessons\" DEFAULT VALUES"
		}
...
func (o VirtualLessonSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf("UPDATE \"virtual_lessons\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, virtualLessonPrimaryKeyColumns, len(o)))
...
func (o *VirtualLesson) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"virtual_lessons\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(virtualLessonType, virtualLessonMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o VirtualLessonSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := "DELETE FROM \"virtual_lessons\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, virtualLessonPrimaryKeyColumns, len(o))
...
func (o *VirtualLessonSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"virtual_lessons\".* FROM \"virtual_lessons\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, virtualLessonPrimaryKeyColumns, len(*o))
