...
var (
	fileDBTypes = map[string]string{`BackupCount`: `smallint`, `ContentUnitID`: `bigint`, `CreatedAt`: `timestamp with time zone`, `FileCreatedAt`: `timestamp with time zone`, `FirstBackupTime`: `timestamp with time zone`, `ID`: `bigint`, `Language`: `character`, `MimeType`: `character varying`, `Name`: `character varying`, `ParentID`: `bigint`, `Properties`: `jsonb`, `Published`: `boolean`, `Secure`: `smallint`, `Sha1`: `bytea`, `Size`: `bigint`, `SubType`: `character varying`, `Type`: `character varying`, `UID`: `character`}
	_           = bytes.MinRead
)
