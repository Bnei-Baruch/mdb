...
func (contentUnitI18nL) LoadContentUnit(e boil.Executor, singular bool, maybeContentUnitI18n interface{}) error {
	...
	if singular {
		...
		slice = *maybeContentUnitI18n.(*ContentUnitI18nSlice)
		count = len(slice)
	}
...
func (o *ContentUnitI18n) Insert(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("INSERT INTO \"content_unit_i18n\" (\"%s\") VALUES (%s)", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.IndexPlaceholders, len(wl), 1, 1))
...
func (o *ContentUnitI18n) Update(exec boil.Executor, whitelist ...string) error {
	...
	if !cached {
		wl := strmangle.UpdateColumnSet(contentUnitI18nColumns, contentUnitI18nPrimaryKeyColumns, whitelist)
		if len(wl) == 0 {
...
func (o ContentUnitI18nSlice) UpdateAll(exec boil.Executor, cols M) error {
	...
	sql := fmt.Sprintf(
		"UPDATE \"content_unit_i18n\" SET %s WHERE (\"content_unit_id\",\"language\") IN (%s)",
		strmangle.SetParamNames("\"", "\"", 1, colNames),
		strmangle.Placeholders(dialect.IndexPlaceholders, len(o)*len(contentUnitI18nPrimaryKeyColumns), len(colNames)+1, len(contentUnitI18nPrimaryKeyColumns)),
	)
...
func (o *ContentUnitI18n) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns []string, whitelist ...string) error {
	...
	if !cached {
		...
		cache.query = queries.BuildUpsertQueryPostgres(dialect, "\"content_unit_i18n\"", updateOnConflict, ret, update, conflict, whitelist)

		cache.valueMapping, err = queries.BindMapping(contentUnitI18nType, contentUnitI18nMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o ContentUnitI18nSlice) DeleteAll(exec boil.Executor) error {
	...
	sql := fmt.Sprintf(
		"DELETE FROM \"content_unit_i18n\" WHERE (%s) IN (%s)",
		strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, contentUnitI18nPrimaryKeyColumns), ","),
		strmangle.Placeholders(dialect.IndexPlaceholders, len(o)*len(contentUnitI18nPrimaryKeyColumns), 1, len(contentUnitI18nPrimaryKeyColumns)),
	)
...
func (o *ContentUnitI18nSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := fmt.Sprintf(
		"SELECT \"content_unit_i18n\".* FROM \"content_unit_i18n\" WHERE (%s) IN (%s)",
		strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, contentUnitI18nPrimaryKeyColumns), ","),
		strmangle.Placeholders(dialect.IndexPlaceholders, len(*o)*len(contentUnitI18nPrimaryKeyColumns), 1, len(contentUnitI18nPrimaryKeyColumns)),
	)
