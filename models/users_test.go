...
var (
	userDBTypes = map[string]string{`AccountID`: `character varying`, `Comments`: `character varying`, `CreatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Email`: `character varying`, `ID`: `bigint`, `Name`: `character varying`, `Phone`: `character varying`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
