...
func testPersonToManyContentUnitsPersons(t *testing.T) {
	...
	slice := PersonSlice{&a}
	if err = a.L.LoadContentUnitsPersons(tx, false, (*[]*Person)(&slice)); err != nil {
		t.Fatal(err)
	}
...
func testPersonToManyPersonI18ns(t *testing.T) {
	...
	slice := PersonSlice{&a}
	if err = a.L.LoadPersonI18ns(tx, false, (*[]*Person)(&slice)); err != nil {
		t.Fatal(err)
	}
...
var (
	personDBTypes = map[string]string{`ID`: `bigint`, `Pattern`: `character varying`, `UID`: `character`}
	_             = bytes.MinRead
)
