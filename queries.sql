...
WITH RECURSIVE rc AS (SELECT c.*, c.id :: TEXT AS path
                      FROM catalogs c
                      WHERE c.id = 40
    UNION
    SELECT c.*, concat(rc.path, '/', c.id) AS path
    FROM catalogs c
           INNER JOIN rc ON c.parent_id = rc.id)
SELECT rc.id, rc.parent_id, rc.path, rc.name
FROM rc
ORDER BY rc.path;
...
SELECT ccu.collection_id, array_agg(DISTINCT cu.type_id)
...
HAVING not (11 = any(array_agg(DISTINCT cu.type_id)))
   and not (21 = any(array_agg(DISTINCT cu.type_id)));
...
WITH RECURSIVE rec_catalogs AS (SELECT c.id
    ...
    FROM catalogs c
           INNER JOIN rec_catalogs rc ON c.parent_id = rc.id)
SELECT DISTINCT cc.container_id
FROM rec_catalogs rc
       INNER JOIN catalogs_containers cc ON rc.id = cc.catalog_id;
...
WITH RECURSIVE rec_catalogs AS (SELECT c.id
    ...
    FROM catalogs c
           INNER JOIN rec_catalogs rc ON c.parent_id = rc.id)
SELECT count(DISTINCT cc.container_id)
FROM rec_catalogs rc
       INNER JOIN catalogs_containers cc ON rc.id = cc.catalog_id;

SELECT fa.id, fa.sha1, array_agg(DISTINCT cfa.container_id)
FROM file_assets fa
       INNER JOIN containers_file_assets cfa ON fa.id = cfa.file_asset_id AND fa.sha1 IS NOT NULL
GROUP BY fa.id;
...
WITH RECURSIVE rec_catalogs AS (SELECT c.id
    ...
    FROM catalogs c
           INNER JOIN rec_catalogs rc ON c.parent_id = rc.id)
SELECT DISTINCT regexp_replace(split_part(fa.name, '.', 1), '^.*_', '') AS publicator
FROM rec_catalogs rc
       INNER JOIN catalogs_containers cc ON rc.id = cc.catalog_id
...
SELECT *
FROM "files"
WHERE (uid = '72824d8e6dd0103d90957854c786d6c47a0056ce')
   OR (id :: TEXT = '72824d8e6dd0103d90957854c786d6c47a0056ce')
   OR (sha1 :: TEXT ~ '72824d8e6dd0103d90957854c786d6c47a0056ce')
   OR (name ~ '72824d8e6dd0103d90957854c786d6c47a0056ce')
ORDER BY id DESC
LIMIT 50;

-- cu with image files in more than one language
SELECT cu.id, cu.uid, cu.type_id, array_agg(DISTINCT f.language)
FROM content_units cu
...
SELECT c.id, c.uid, ct.name, c.created_at, c.properties, concat('http://app.mdb.bbdomain.org/admin/collections/', c.id)
FROM collections c
...
select cu.id, ccu.position, ccu.name, concat('https://archive.kbb1.com/he/programs/cu/', cu.uid)
from content_units cu
...
FROM (select cu.id, cu.properties ->> 'film_date', ccu.position, ROW_NUMBER()
    OVER (
      ORDER BY cu.properties ->> 'film_date') as row_number
      from collections_content_units ccu
             inner join content_units cu on ccu.content_unit_id = cu.id and ccu.collection_id = 10560
      order by cu.properties ->> 'film_date') AS s
WHERE content_unit_id = s.id
  and collection_id = 10560;
...
select cu.id, cu.uid, ct.name, cu.properties ->> 'film_date' as "film_date", cui.name
from content_units cu
       inner join content_types ct on cu.type_id = ct.id
       left join content_unit_i18n cui on cu.id = cui.content_unit_id and cui.language = 'ru'
where cu.properties ->> 'original_language' = 'ru'
  and cu.published is true
  and cu.type_id != 44
  and cu.id not in (select distinct cu.id
                    from content_units cu
                           inner join files f on cu.id = f.content_unit_id and
...
-- add last update workflow id from send operation to unit
-- based on parsing the file name
SELECT DISTINCT ON (cu.id) cu.id, cu.uid, (regexp_matches(f.name, '.*_([t|d][\d+]*)o\.mp4')) [ 1 ] AS wfid
FROM operations o
       INNER JOIN files_operations fo ON o.id = fo.operation_id AND o.type_id = 4
       INNER JOIN files f ON fo.file_id = f.id AND f.content_unit_id IS NOT NULL and f.name like '%o.mp4'
       inner join content_units cu on f.content_unit_id = cu.id
...
select f.id, f.name, f.content_unit_id, f.type, f.language, f.properties ->> 'kmedia_id' as kmid, f.sha1
from files f
       left join files_storages fs on f.id = fs.file_id and fs.storage_id = 214
...
-- files for ffprobe scan job
select f.id, f.name, f.type, encode(f.sha1, 'hex')
from files f
       inner join files_storages fs on f.id = fs.file_id and fs.storage_id = 214
where f.published is true
  and f.secure = 0
  and f.sha1 is not null
  and f.type in ('audio', 'video');

-- parents to exclude
select id, name, created_at, secure, published, content_unit_id
from files
where id in (select distinct parent_id from files)
  and name not like '%flv'
  and name not like '%wmv'
order by name;
...
-- select count(*), sum(size) / (1024 ^ 4)
select id, name,  content_unit_id, published
from files f
       inner join files_storages fs
         on f.id = fs.file_id and fs.storage_id in (select id from storages where location = 'merkaz'
                                                                              and status = 'online')
where removed_at is null
  and (f.name like '%wmv'
         or f.name like '%flv'
         or (f.id not in (select distinct parent_id from files where parent_id is not null)))
  and (f.content_unit_id is null or published is false)
order by f.name;
...
select concat('https://kabbalahmedia.info/he/programs/cu/', cu.uid) as link, cui.name
from content_units cu
       inner join content_unit_i18n cui on cu.id = cui.content_unit_id and cui.language = 'he'
where cu.type_id = 20;

select concat('https://kabbalahmedia.info/he/programs/c/', c.uid) as link, ci.name
from collections c
       inner join collection_i18n ci on c.id = ci.collection_id and ci.language = 'he'
where c.type_id = 5;

select count(distinct fs.file_id)
from operations o
       inner join files_operations fo on o.id = fo.operation_id and o.type_id = 11
       inner join files_storages fs on fo.file_id = fs.file_id
;



select cu.id, f.id, f.name, cu.properties ->> 'duration', f.properties ->> 'duration'
from content_units cu
       inner join files f on cu.id = f.content_unit_id
where f.type in ('audio', 'video')
--   and cu.properties ? 'duration'
--   and f.properties ? 'duration'
  and f.properties ? 'duration' != cu.properties ? 'duration'
order by cu.id, f.id
limit 100;

select content_unit_id, left(substr(name, 7), -4), count(*)
from files
where type in ('audio', 'video')
group by content_unit_id, left(substr(name, 7), -4)
having count(*) > 1
order by content_unit_id, left(substr(name, 7), -4), count(*) desc;
...
where cu.type_id = 20
  and f.name ~ 'clip'
  and cu.id in (select distinct cu.id
                from content_units cu
                       inner join files f on cu.id = f.content_unit_id
                where cu.type_id = 20
                  and f.name ~ '_rav_.*program')
order by cu.id desc;

-- select count(distinct content_unit_id) from files where type = 'image'
...
from content_units cu
       inner join content_units_sources cus on cu.id = cus.content_unit_id and cus.source_id = 8
       inner join files f on cu.id = f.content_unit_id and f.type = 'image' and f.language != 'en'
order by cu.properties ->> 'film_date';