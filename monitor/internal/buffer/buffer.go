...
func NewBuffer(size int) *Buffer {
	return &Buffer{
		buf: make(chan interfaces.Metric, size),
	}
}
...
func (b *Buffer) Add(metrics ...interfaces.Metric) {
...
func (b *Buffer) Batch(batchSize int) []interfaces.Metric {
	b.mu.Lock()
	n := min(len(b.buf), batchSize)
	out := make([]interfaces.Metric, n)
	for i := 0; i < n; i++ {
		out[i] = <-b.buf
	}
	b.mu.Unlock()
