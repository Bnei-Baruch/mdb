...
func (c *CPUStats) TryParseConfigurations(inputConfigs map[string]interface{}) error {
	log.Printf("Parsing CPU input configurations")
	mapstructure.Decode(inputConfigs, &c)
...
func (s *CPUStats) Gather(acc interfaces.Accumulator) error {
	log.Println("Gathering CPU statistics...")
...
func init() {
	inputs.Add("cpu", func() interfaces.Input {
		return &CPUStats{
			PerCPU:   true,
			TotalCPU: true,
			ps:       newSystemPS(),
